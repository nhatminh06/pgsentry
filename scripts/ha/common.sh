...
wait_cluster() { local state leaders replicas; for _ in $(seq 1 60); do state=$(cluster_json 2>/dev/null || true); leaders=$(jq '[.[]|select(.Role=="Leader" and .State=="running")]|length' <<<"${state:-[]}" 2>/dev/null || echo 0); replicas=$(jq '[.[]|select(.Role|test("Replica"))|select(.State=="streaming")]|length' <<<"${state:-[]}" 2>/dev/null || echo 0); [[ $leaders -eq 1 && $replicas -eq 2 ]] && return; sleep 3; done; patronictl list || true; echo 'Patroni cluster did not reach 1 leader + 2 streaming replicas' >&2; return 1; }
